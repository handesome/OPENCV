...
def threshold_simple(image):
    ...
    ret, thresh1 = cv.threshold(img, 127, 255, cv.THRESH_BINARY)#大于127为白，小于127为黑
    ret, thresh2 = cv.threshold(img, 127, 255, cv.THRESH_BINARY_INV)#小于127为白
    ret, thresh3 = cv.threshold(img, 127, 255, cv.THRESH_TRUNC)
    ret, thresh4 = cv.threshold(img, 127, 255, cv.THRESH_TOZERO)#小于阈值自动归零
...
def threshold_adaptive(image):
    ...
    th2 = cv.adaptiveThreshold(img, 255, cv.ADAPTIVE_THRESH_MEAN_C, cv.THRESH_BINARY, 11, 2)#参数倒数第二个为奇数，如11，最后一个为常量，减弱噪声，可更改
...
def big_image_demo(image):
    ...
    for row in range(0, h, ch):
        for col in range(0, w, cw):
            ...
            dst = cv.adaptiveThreshold(roi, 255, cv.ADAPTIVE_THRESH_GAUSSIAN_C, cv.THRESH_BINARY, 127, 2)#高斯
...
def main():
    img = cv.imread(r"d:\python\lena.jpg")
    # threshold_demo(img)
